...
favicon + unread dot + auto-pin/dedup). But you still have to _manually open_ each PR. For repos
...
This feature lets the user mark repos as **watched**; when a _new_ PR is opened in a watched repo,
...
| **Renovate** _(hardcoded)_                                                                      | PRs authored by the Renovate bot are skipped.                                                                                                                        |
| **Only-after-add** _(hardcoded)_                                                                | Only PRs **created after** the repo was added to the watch list are opened — never the pre-existing backlog.                                                         |
...
- **W2.** **Only-after-add watermark — by PR number, not clock.** When a repo is added, do one list
  ...
  opened and is _implicitly_ handled — no per-PR storage needed for it (see W6).
...
- **W4a.** **Alarm lifecycle (change to existing code).** The poll alarm is today created only while
  there are pollable PR _tabs_ (`hasPollable(prRegistry)`) and cleared when none are open. Watched-
...
- **W5.** **Skip Renovate** _(hardcoded)_ — a PR whose author login matches Renovate
  (case-insensitive `/renovate/`, covering `renovate[bot]` and common self-hosted `renovate` /
  `renovate-bot` logins), via a named constant so variants are a one-line add. _(A more robust
  structural signal — `author.__typename === "Bot"` in the GraphQL Actor type — is noted for later;
  v1 deliberately filters Renovate specifically, not all bots.)_
- **W5a.** **Skip drafts until ready** _(decided v1)_ — a PR that is a **draft** at detection is not
...
- **W6.** **Open-once / respect-close.** A PR is auto-opened **at most once**. Persist a per-repo
  **handled set** of PR numbers the extension opened, in **`chrome.storage.local`** (survives restart
  — _not_ `session`), so a closed auto-opened PR is **not re-opened** next tick. Numbers **≤ the
  ...
  filtered by W5/W2, not stored individually). _Known v1 limitation:_ a PR you reviewed-and-closed
...
- **W7.** **Don't duplicate an already-open PR.** Before opening, check whether the PR is already
  ...
  `storage.session` — _all_ tabs right after a restart). Also guard against double-open **within a
...
- **W8a.** **Focus discipline is an assumption to verify.** "Never steal focus" rests on
  ...
  existing code only ever creates inactive tabs _without_ a `windowId` (current window), so the
...
- PRs that were already open in the repo _before_ you added it are **never** auto-opened.
...
- **No _interruptive_ notifications** — no OS toast, sound, badge count, or focus-stealing. Auto-
  opening an inactive tab is the passive delivery channel (the deliberate-inbox direction reframes
  the unread doc's "not a notification system" line as "no _interruptive_ notifications").
...
- **Reuse the existing alarm + token + auto-pin/dedup/favicon stack** — watched-repo polling is a
  new _list_ query layered onto the existing background poller; opened tabs are just PR tabs, so the
...
- **Deliberate inbox direction** — auto-opening tabs is owned as a _passive PR inbox_; the prior
  "not a notification system" boundary is reframed as "no _interruptive_ notifications," so future
